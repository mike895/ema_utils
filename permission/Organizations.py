...
class CustomDatabaseQuery(DatabaseQuery): 
    def build_and_run(self):   # all_regions = all_branch and region_item = branch and regions = branchs
        ...
        query = f"""select *
            from %(tables)s
            %(conditions)s
            {f'''{"AND " if(self.conditions.__len__() > 0) else ""}NOT EXISTS(select 1 from `tabSelected Branch` 
                    where `tabSelected Branch`.`parent` = `tabOrganizations`.`name`
                    AND  `tabSelected Branch`.`branch` NOT IN ({self.branchs}))''' if (self.branchs != "all") else ""}
            %(group_by)s
            %(order_by)s
            %(limit)s""" % args
...
def getcount(permission):
    ...
    args.fields = [f"count({distinct}`tabOrganizations`.name) as total_count"]
    dbq = CustomDatabaseQuery("Organizations")
    dbq.branchs = allowed_branchs
...
def getdoc(name, permission):
    ...
    try:
        ...
        helper.is_allowed(permission, [i.branch for i in doc.branch], throw=True, all_match=False)
        load.run_onload(doc)
