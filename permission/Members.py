    ...
    def build_and_run(self):   # all_regions = all_branch and region_item = branch and regions = branchs
        ...
        query = f"""select %(fields)s
            ...
            {f'''{"AND " if(self.conditions.__len__() > 0) else ""}NOT EXISTS(select 1 from `tabSelected Branch` 
                    where `tabSelected Branch`.`parent` = `tabMembers`.`name`
...
def getcount(permission):
    ...
    args.fields = [f"count({distinct}`tabMembers`.name) as total_count"]
    dbq = CustomDatabaseQuery("Members")
    dbq.branchs = allowed_branchs
